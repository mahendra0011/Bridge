...
import {
  ShieldCheck, Upload, CheckCircle2, Clock, AlertCircle, AlertTriangle,
  X, FileText, Building2, ArrowRight, BadgeCheck, Info
} from 'lucide-react'
...
export default function AgencyVerification() {
  ...
  const loadVerification = () => {
    setLoading(true)
    api.get('/api/agency/verification').then(data => {
  ...
  const handleUpload = async () => {
    ...
    formData.append('type', 'udyam')
    try {
      const data = await api.upload('/api/agency/verification/upload', formData)
      setVerification(data.verification || data)
  ...
  const isVerified = agency?.isVerified || verification?.isVerified
  const status = verification?.status || (isVerified ? 'verified' : 'unverified')
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Verify your agency to build trust and unlock unlimited postings.</p>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-lg font-bold">
                {status === 'verified' ? 'Verified Agency'
                  : status === 'pending' ? 'Verification in Progress'
                  : 'Not Yet Verified'}
              </h3>
              <p className="mt-1 text-sm text-slate-600">
                {status === 'verified'
                  ? 'Your agency is verified. You have full access to all features including unlimited postings.'
                  : status === 'pending'
                  ? 'Your documents are under review. This usually takes 1-2 business days.'
                  : 'Verify your agency with Udyam registration or other official documents to unlock unlimited postings and build trust with clients.'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            <div className="flex items-start gap-4 px-6 py-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-sm">Complete Agency Profile</p>
                <p className="text-xs text-slate-500">Fill in your agency details, description, and contact information.</p>
              </div>
              {agency?.isProfileComplete ? (
                <CheckCircle2 className="size-5 text-emerald-500 shrink-0" />
              ) : (
                <Link to="/agency/profile" className="text-xs font-bold text-primary hover:underline shrink-0">Complete →</Link>
              )}
            </div>
            <div className="flex items-start gap-4 px-6 py-4">
              <div className={`grid size-8 shrink-0 place-items-center rounded-full text-sm font-bold ${
              ...
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-sm">Upload Udyam / Business Registration</p>
                <p className="text-xs text-slate-500">Upload your Udyam registration certificate or other official business documents.</p>
              </div>
              {status === 'verified' ? (
                <CheckCircle2 className="size-5 text-emerald-500 shrink-0" />
              ) : (
            ...
            <div className="flex items-start gap-4 px-6 py-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500">Our team reviews your documents and verifies your agency.</p>
              </div>
              {status === 'verified' ? (
                <CheckCircle2 className="size-5 text-emerald-500 shrink-0" />
              ) : (
        ...
        {/* Upgrade from Informal Team */}
        {!agency?.isRegistered && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50/50 p-6">
            <div className="flex items-start gap-3">
              <AlertTriangle className="size-6 shrink-0 text-amber-600 mt-0.5" />
              <div>
                <h3 className="font-bold text-amber-800">Informal Team — Upgrade to Verified</h3>
                <p className="mt-1 text-sm text-amber-700">
                  You're currently operating as an informal team with limited posting capacity ({agency?.unregisteredLimit || 3} per month).
                  Register your agency with Udyam and complete verification to unlock unlimited postings, 
                  get a "Verified" badge on your profile, and access advanced features.
                </p>
                <div className="mt-4 flex gap-3">
                  <Link to="/agency/profile" className="inline-flex items-center gap-1.5 rounded-xl bg-amber-600 px-4 py-2 text-sm font-bold text-white hover:bg-amber-700">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { icon: BadgeCheck, label: 'Verified Badge', desc: 'Get a trust-building verified badge on your profile' },
              { icon: Upload, label: 'Unlimited Postings', desc: 'Post unlimited jobs, internships, and gigs' },
              { icon: ShieldCheck, label: 'Higher Visibility', desc: 'Verified agencies rank higher in search results' },
            ].map((item, i) => (
