...
import {
  ...
  CalendarDays, Building2, ExternalLink, Eye,
} from 'lucide-react'
...
export function InternshipCard({ item, index = 0 }) {
  ...
  const { handleSave } = useCardActions('internship')
  ...
  return (
    <CardWrapper isClosed={isClosed} isBoosted={isBoosted} index={index}>
        ...
        >
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            {item.companyVerified && (
              <BadgeCheck className="size-3.5 shrink-0 text-blue-500 sm:size-4" />
            )}
          </div>
        </Link>
      ...
      <div className="mt-2.5 sm:mt-3 flex items-start gap-2">
        ...
        <span className="inline-flex items-center rounded-md bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700 ring-1 ring-emerald-200 sm:px-2.5 sm:text-[11px]">
          Internship
        </span>
      </div>
      ...
      {/* Row 4: Duration · Start date — PPO tag */}
      <div className="mt-2 flex flex-wrap items-center gap-2">
        {item.duration && (
          <span className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600 sm:text-[11px]">
            <CalendarDays className="size-3" /> {item.duration}
          </span>
        )}
        {item.startDate && !item.startDate.raw?.includes('Invalid') && (
          <span className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600 sm:text-[11px]">
            <Clock className="size-3" /> {item.startDate.label}
          </span>
        )}
        {item.hasPPO && (
          <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-700 ring-1 ring-amber-200 sm:px-2.5 sm:text-[11px]">
            Internship with PPO
          </span>
        )}
        {item.vacancies > 1 && (
      ...
      {/* Row 6: Stipend + Deadline + Posted + Apply */}
      <div className="mt-3 flex items-center gap-3 border-t border-slate-100 pt-3 sm:mt-4 sm:pt-4">
        <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-[10px] font-bold ring-1 sm:px-3 sm:py-1 sm:text-xs ${
          item.pay === 'Unpaid' || item.pay === 'Not disclosed'
